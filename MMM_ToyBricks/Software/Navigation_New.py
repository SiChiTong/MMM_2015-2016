...
def handToPosition(robot, x, y, z, hand = None, debug = False):
    ...
    arm_extension = arm - ARM_LENGTH_MIN
...
def default_test(debug = False):
    ...
    handToPosition(mmm, SHOULDER_WIDTH, 0.4, 0.1, "right", debug)
    if debug == False: mmm.update()
    ...
    raw_input("Press Enter.") 
    print "Raise arms & adjust wrist"
    handToPosition(mmm, SHOULDER_WIDTH+.05, 0.45, 0, "right", debug)
    if debug == False: mmm.update()

    raw_input("Press Enter.") 
    print "Raise arms & adjust wrist"
    handToPosition(mmm, SHOULDER_WIDTH+.1, 0.45, .05, "right", debug)
    if debug == False: mmm.update()
